...
public class Program {
    public static void Main(string[] args) {
        ...
        else if (command == "ffvi-auto-battle") {
            doFf6AutoBattle();
        }
        // else if (command == "save-client") doSaveClient();
        // else if (command == "log-screen") doLogScreen(dict);
        // else if (command == "test") doTest(dict);
        else if (command == "crisis-core") {
            doCrisisCoreTest();
        }
        else if (command == "ffvii-super-dunk") {
            doFf7SuperDunk();
        }
        else if (command == "ffvii-farm") {
            doFf7Farm();
        }
        else if (command == "ffix-farm") {
            doFf9Farm();
        }
        else if (command == "ffix-grand-dragon") {
            doFf9GrandDragon();
        }
            ...
            doGetTextBasedOnCursor(dict);
        }
        else if (command == "r1-turbo") {
            doR1Turbo(dict);
        }
    }

    private static void doR1Turbo(Dictionary<string, string> dict) {
        new CustomControllerUser(new RealStopwatch(), GetConnectedController()).Create();
    }
    ...
    private static void doFf9GrandDragon() {
        // Client is 634x371
        RealStopwatch s = new();
        var user = GetStopwatchControllerUser(s, 100, 50);
        ...
        void DoUntilTextComes(int x, int y, string text, Action action) {
            while (!TextContainsWord(x, y, text)) {
                action();
            }
        }
        ...
        while (true) {
            ...
            // Tent up
            void WaitForTent() =>
                DoUntilTextComes(236, 105, "Tent", () => { user.PressButton(ButtonMappings.Square); });
    ...
    private static void doFf6AutoBattle() {
        ...
        var lastPressedDpad = DPadMappings.Left;
        while (true) {
            var dpadToPress =
    ...
    private static void doLogCursor(Dictionary<string, string> dict) {
        ...
        while (true) {
            Point point = new(0, 0);
            var hasCoordinates = dict.ContainsKey("X") && dict.ContainsKey("Y");
            ...
            var message = bw.ProcessBitmap(
                point.X,
                point.Y,
                bm => {
                    var s = new Pixel(point.X, point.Y, bw.GetAverageColor(bm)).ToString();
                    if (client != null) {
                        User32.ScreenToClient(client, ref point);
    ...
    private static void doRecord(Dictionary<string, string> dict) {
        var processName = dict["processName"];
        User32.RestoreWindow(processName);
        var countParam = int.Parse(dict["count"]);
        // int countParam = 1;
        var durationParam = double.Parse(dict["duration"]);
        // double durationParam = 0;
        Console.WriteLine(
            $"ProcessName param - {processName}, count param - {countParam}, duration param - {durationParam}"
        );

        var directory = $"C:\\D\\Apps\\Vigem_scripts\\Recordings\\{DateTime.Now:yyyy-MM-dd--HH-mm-ss}";
        ...
        localStopwatch.Restart();
        while (true) {
            var filePath = $"{directory}\\{counter}-{DateTime.Now:yyyy-MM-dd--HH-mm-ss.fff}.png";
            bw.ProcessBitmap(
                processName,
                bm => {
    ...
    private static StopwatchControllerUser GetStopwatchControllerUser(
        IStopwatch s,
        int pressLength,
        int delayAfterSet = 0
    ) => new(GetConnectedController(), s, pressLength, delayAfterSet);

    private static Dualshock4Controller GetConnectedController() {
