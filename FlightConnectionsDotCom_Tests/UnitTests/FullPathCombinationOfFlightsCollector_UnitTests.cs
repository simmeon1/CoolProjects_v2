...
namespace FlightConnectionsDotCom_Tests.UnitTests
{
    ...
    {        
        ...
        public void GetFullPathCombinationOfFLights_MultipleFlights()
        {
            ...
            List<KeyValuePair<Path, List<Flight>>> data = new();
            data.Add(new KeyValuePair<Path, List<Flight>>(path1, new List<Flight>() { flight1, flight2 }));
            data.Add(new KeyValuePair<Path, List<Flight>>(path2, new List<Flight>() { flight3, flight4 }));
            data.Add(new KeyValuePair<Path, List<Flight>>(path3, new List<Flight>() { flight5, flight6, flight7 }));

            FullPathCombinationOfFlightsCollector collector = new(new KeyValuePair<Path, List<KeyValuePair<Path, List<Flight>>>>(allPath, data));
            List<List<Flight>> fullPathCombinationOfFlights = collector.GetFullPathCombinationOfFLights();
        ...
        public void GetFullPathCombinationOfFLights_OneFlight()
        {
            ...
            List<KeyValuePair<Path, List<Flight>>> data = new();
            data.Add(new KeyValuePair<Path, List<Flight>>(path1, new List<Flight>() { flight1 }));
