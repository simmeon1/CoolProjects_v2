...
{
    public static List<Dictionary<string, List<BillboardSong>>> GetDateSongMaps(
        params List<List<BillboardList>> listOfBillBoardLists
    )
    {
        ...
        List<Dictionary<string, List<BillboardSong>>> dateSongMaps =
            [..listOfBillBoardLists.Select(GetDictFromBillboardList)];
    ...
    private static void MakeSongsMoreGeneric(List<Dictionary<string, List<BillboardSong>>> dateSongMaps)
    {
        ...
        {
            // var songss = songMap.Values.SelectMany(x => x.Select(y => y.song)).Distinct();
            // var artists = songMap.Values.SelectMany(x => x.Select(y => y.artist)).Distinct();
            // var all = songss.Concat(artists)
            //     .SelectMany(x => x.Split(" ", StringSplitOptions.RemoveEmptyEntries))
            //     .GroupBy(x => x, (x, y) => new KeyValuePair<string,int>(x, y.Count()))
