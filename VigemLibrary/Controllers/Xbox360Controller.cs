...
public class Xbox360Controller : IController {
    ...
    public Xbox360Controller(IXbox360Controller controller) => this.controller = controller;
    ...
    public void SetAxisState(AxisMappings axis, byte value) {
        ...
        short result = 0;
        if (x != 0) {
            var byteIsPositive = x >= 1;
            result = byteIsPositive
                ? Convert.ToInt16(short.MaxValue / 127.0 * x)
                : Convert.ToInt16(-(short.MinValue / 128.0 * x));
            if (axis is AxisMappings.LeftThumbY or AxisMappings.RightThumbY) {
                var multiplier = byteIsPositive
                    ? (double) short.MinValue / short.MaxValue
                    : (double) short.MaxValue / short.MinValue;
    ...
    public ControllerState GetState() => throw new NotImplementedException();
